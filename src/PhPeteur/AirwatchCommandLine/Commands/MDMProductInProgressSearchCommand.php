...
use PhPeteur\AirwatchWebservices\Services\AirwatchMDMProductInProgressSearch;
 ...
 */
class MDMProductInProgressSearchCommand extends AirwatchCmd
{
    protected function configure()
    {
        $this->_oAW = new AirwatchMDMProductInProgressSearch($this->_config);
        if (is_null($this->_oAW))
            throw new AirwatchCmdException('Unable to create AirwatchMDMProductsFailedSearch object within' . __CLASS__, 42);

        $this->setName('mdm-productinprogress-search');
        ...
        $this->setDescription('Returns the details of the devices for which Provisioning is In-Progress.');
