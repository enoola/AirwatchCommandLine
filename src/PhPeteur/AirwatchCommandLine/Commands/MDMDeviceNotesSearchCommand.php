...
use PhPeteur\AirwatchWebservices\Services\AirwatchMDMDeviceNotesSearch;

/*
 * Retrieve Notes From the Device
 * Functionality – Retrieves details of all the notes from the device.
 */
class MDMDeviceNotesSearchCommand extends AirwatchCmd
{
    protected function configure()
    {
        $this->_oAW = new AirwatchMDMDeviceNotesSearch($this->_config);
        if (is_null($this->_oAW))
            die ("Unable to create AirwatchMDMDeviceNotesSearch object :/");

        $this->setName('mdm-device-notes-search');
