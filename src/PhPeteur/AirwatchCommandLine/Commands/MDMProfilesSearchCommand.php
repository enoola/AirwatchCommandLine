...
use PhPeteur\AirwatchWebservices\Services\AirwatchMDMProfilesSearch;

class MDMProfilesSearchCommand extends AirwatchCmd
{

    protected function configure()
    {
        $this->_oAW = new AirwatchMDMProfilesSearch( $this->_config );
        if (is_null( $this->_oAW) )
            throw new AirwatchCmdException('Unable to create AirwatchMDMProfilesSearch object within'.__CLASS__,42);

        $this->setName('mdm-profiles-search');
        if (! is_null ( $this->_oAW->getPossibleSearchParams() ) ) {
            foreach ($this->_oAW->getPossibleSearchParams() as $param => $pdescription) {
                $this->addOption($param, null, InputOption::VALUE_REQUIRED, $pdescription);
            }
        }
        $this->setDescription(AirwatchMDMProfilesSearch::CLASS_SENTENCE_AIM);
