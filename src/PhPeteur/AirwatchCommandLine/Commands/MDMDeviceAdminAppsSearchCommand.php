...
use PhPeteur\AirwatchWebservices\Services\AirwatchMDMDeviceAdminAppsSearch;

/*
 * Retrieve Admin Application Details
 * Functionality – Retrieves admin applications details for the passed device ID.
 */
class MDMDeviceAdminAppsSearchCommand extends AirwatchCmd
{
    protected function configure()
    {
        $this->_oAW = new AirwatchMDMDeviceAdminAppsSearch( $this->_config );
        if (is_null( $this->_oAW))
            die (">>unable to create AirwatchMDMDeviceAdminAppsSearch object :/");

        $this->setName('mdm-device-adminapps-search');
