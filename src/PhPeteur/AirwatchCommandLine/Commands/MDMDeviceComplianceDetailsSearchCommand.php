...
use PhPeteur\AirwatchWebservices\Services\AirwatchMDMDeviceComplianceDetailsSearch;

class MDMDeviceComplianceDetailsSearchCommand extends AirwatchCmd
{

    protected function configure()
    {
        //$this->_fieldnameToShowInDataResult = "Application";
        $this->_oAW = new AirwatchMDMDeviceComplianceDetailsSearch( $this->_config );
        if (is_null( $this->_oAW))
            die ("Unable to create AirwatchMDMDevicesSearch object :/");

        $this->setName('mdm-device-compliancedetails-search');
