...
use PhPeteur\AirwatchWebservices\Services\AirwatchMAMAppsRemovalLogsSearch;

class MAMAppsRemovalLogsSearchCommand extends AirwatchCmd
{

    protected function configure()
    {
        $this->_oAW = new AirwatchMAMAppsRemovalLogsSearch($this->_config);
        if (is_null($this->_oAW))
            throw new AirwatchCmdException('Unable to create AirwatchMAMAppsRemovalLogsSearch' . __CLASS__, 42);

        $this->setName('mam-appsremovallogs-search');
